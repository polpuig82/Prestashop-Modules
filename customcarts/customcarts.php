...
// Load current value
    ...
    public function hookDisplayHeader($params)
    {
        ...
        $this->context->controller->addCSS($this->_path . 'views/css/front-mini-23-02-23.css');
        ...
        $this->context->controller->addCSS($this->_path . 'views/css/custom.css');
         ...
         $this->context->controller->addJS($this->_path . 'views/js/front-mini-23-02-23.js');
    ...
    public function hookDisplayCestaPersonalizada($params)
    {
        ...
        $id_attribute = 0;
        if(isset($params['product']['id_attribute'])) {
            $id_attribute = $params['product']['id_attribute'];
        }
        ...
        $productosCestaDefinida=customCartsOb::getProductosCestaDefinida($id_product,$id_lang,$id_attribute);
        $productosCestaAlternativa=customCartsOb::getProductosCestaAlternativa($id_product,$id_lang,$id_attribute);
            // Valor por defecto
            $incrementoAlternativo = 1.00;

            // Si está logueado, usamos su grupo
            if ($this->context->customer->isLogged()) {
                $defaultGroupId = (int) $this->context->customer->id_default_group;
            } else {
                // Si no está logueado, usamos el grupo de visitantes
                $defaultGroupId = (int) Configuration::get('PS_UNIDENTIFIED_GROUP');
            }

            // Obtener el margen configurado para ese grupo
            $id_priceC = Db::getInstance()->getValue(
                'SELECT id_priceC FROM ' . _DB_PREFIX_ . 'group WHERE id_group = ' . $defaultGroupId
            );

            // Si es válido, lo aplicamos
            if ($id_priceC !== false && $id_priceC > 0) {
                $incrementoAlternativo = (float) $id_priceC;
            }

            // Asignar a Smarty
            $this->context->smarty->assign('incremento_contenido_alternativo', $incrementoAlternativo);
        ...
        if(count($productosCestaDefinida)>0) {
            ...
            return $this->display(__FILE__, 'views/templates/front/mini-product.tpl');
        }
    }
    ...
    private function installSQL()
    {
        ...
        $sql .= 'CREATE TABLE IF NOT EXISTS`' . _DB_PREFIX_ . 'customcarts_table` (
                    ...
                    `unidades_aprox` varchar(20) NOT NULL,
                    `origen` varchar(40),
                    `orden` int(10),
                    PRIMARY KEY (`id`)
                    ) ENGINE=' . _MYSQL_ENGINE_ . ' DEFAULT CHARSET=utf8;';

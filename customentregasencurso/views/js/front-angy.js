...
alert('function comprobarTotal '+id);
        const collection = document.getElementById(id).getElementsByClassName("definido");
        for (let i = 0; i < collection.length; i++) {
...
function changeItemFromBasket(posicion, idCambio, num, contadorLinea, padre, linea, obId, impactoPrecio) {
    ...
    var espacio = "&nbsp";
    idElemento = idCambio + "-" + num + "-" + contadorLinea;
...
alert(idDesh);    
    ...
    if (totalPedidosEnCurso != 6) {
        ...
        //comprobarTotal('contentcesta ' + padre + '-' + num + linea, window["cambiosCesta" + num + linea]);
        desplegarcarrouselProductoCesta(idElemento);
        //15-06-2022 cambiarId(linea,idCambio,num,nodoCesta[posicion].id,padre);
        cambiarId(contadorLinea, idCambio, num, linea, nodoCesta[posicion].id, padre);
...
function undoChangeItemFromBasket(idDesh, idCambio, posicion, general, idElemento, padre, contadorLinea, linea, obId, impactoPrecio) {
    posicion = posicion - 1;
